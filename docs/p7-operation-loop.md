...
9. Guarded Kubernetes smoke covers the same runtime-affecting paths after image
   publish and deployment rollout.
10. Post-smoke cleanup restores mutating flags to default-off and leaves deployment controller
    `Synced / Healthy`.
...
| Internal rollout | private deployment smoke window runbook | image publish, deployment rollout rev, cleanup rev, deployment controller `Synced / Healthy`, default-off cleanup report |
...
default-off/execution/observation/recovery/restart/soak reports, the deployment
rollout/default-off cleanup report, the guarded Kubernetes operation reports for
...
cargo xt p6-rollout-report \
  ...
  --namespace tessera \
  --image registry.example.com/example/tessera:v2026.05.6 \
  --rollout-revision <private-rollout-revision> \
  --cleanup-revision <private-cleanup-revision> \
  --image-published \
...
cargo xt p6-rollout-report-check \
  --expected-image registry.example.com/example/tessera:v2026.05.6
cargo xt p7-completion-audit --json
```

The `v2026.05.6` rollout carries the completed P7
ledger/executor/observation code and the internal canonical multi-depth
operation helper. The default-off rollout at deployment revision `<private-rollout-revision>` promoted
all Tessera deployments to
`registry.example.com/example/tessera:v2026.05.6`. The final cleanup
revision `<private-cleanup-revision>` removed manual operation execution, split/merge activation,
and preview fixture settings while keeping only non-mutating config/state/ledger
paths on the live Orchestrator. After cleanup, deployment controller `tessera` was `Synced /
...
cargo xt k8s operation-report-check \
  --report .dev/reports/guarded-kubernetes-p7-multi-depth-operation-smoke-latest.json \
  --expected-image registry.example.com/example/tessera:v2026.05.6 \
  --require-published-execution \
  --require-completed-observation \
  --require-soak
...
deployment controller `Synced / Healthy` image evidence for `v2026.05.6`. The final
...
`.dev/reports/guarded-kubernetes-p7-operation-smoke-latest.json` with
deployment controller/image/ledger evidence. The approved helper additionally port-forwards
...
internal smoke window should use a fresh operation ledger path in the deployment
...
7. For runtime-affecting slices, publish an image, promote through deployment, wait
   for deployment controller `Synced / Healthy`, run internal smoke, write verifier reports,
...
20. **Internal operation helper**: add `cargo xt k8s operation-smoke` and
   `cargo xt k8s operation-report-check` so guarded Kubernetes can record P7
   proposal evidence by default and approved execution/observation/soak
   evidence during a controlled smoke window.
21. **Internal rollout**: repeat the controlled image/deployment/smoke/cleanup flow
...
- Do not count local smoke as guarded Kubernetes evidence.
- Do not count deployment controller `Synced / Healthy` as runtime success without the
  operation-specific smoke report.
- Keep docs, verifier contracts, and deployment cleanup state synchronized with the
