...
Historical deployment decisions are summarized here without retaining private
environment inventory or rollout instructions.
...
## P4.2 Private Deployment Validation

Status: complete as of 2026-05-01.

Completed outcome:

- Validated the Orchestrator, Worker, and Gateway runtime in a private
  Kubernetes deployment while keeping the custom runtime protocol non-public.
- Verified readiness, client ping, metrics, and default-off split/merge preview
  behavior.
- Kept environment identity, registry coordinates, deployment manifests,
  topology inventory, and rollout provenance in the private deployment source
  of truth.
...
13. Added `cargo xt k8s activation-smoke`, which runs the guarded Kubernetes
...
14. Published `registry.example.com/example/tessera:v2026.05.2`, applied
    the controlled two-Worker deployment smoke topology, verified internal
    Kubernetes split publish/failure/recovery with `cargo xt k8s
...
15. Closed the controlled smoke window through a follow-up private deployment cleanup
...
- Runtime merge activation implementation, automatic planner submission, and
  ...
  operator recovery plus deployment backout, not automatic merge rollback.
...
cargo xt k8s activation-smoke --context example-cluster --namespace tessera --expected-image registry.example.com/example/tessera:v2026.05.2 --allow-activation --with-failure --allow-scale
cargo xt k8s activation-report-check --require-published --require-failure --expected-image registry.example.com/example/tessera:v2026.05.2
cargo xt dev up --with-orch
cargo run -p tessera-client -- ping --ts 123
cargo xt dev down --with-orch
```
...
Use `docs/todo-next.md` for the current open-work index. Historical deployment
decisions remain closed unless `docs/todo-next.md` explicitly reopens them.
