...
5. Guarded Kubernetes split success, failure, and recovery paths are
...
7. The P5 rollback policy is explicit: automatic rollback is disabled, target
   Worker restoration is the recovery path, deployment backout reverts the
...
| Guarded Kubernetes read-only preflight | `cargo xt k8s activation-smoke --context example-cluster --namespace tessera ...`; `.dev/reports/guarded-kubernetes-activation-smoke-latest.json` | Covered |
| Guarded Kubernetes split publish | `cargo xt k8s activation-smoke --allow-activation` against approved two-Worker topology | Covered in internal cluster |
| Guarded Kubernetes failure/recovery | `cargo xt k8s activation-smoke --allow-activation --with-failure --allow-scale` | Covered in internal cluster |
| deployment controller readiness gate before cluster smoke | `cargo xt k8s activation-smoke` checks Application `Synced / Healthy` unless `--skip-argocd-check` is passed | Covered by helper |
...
cargo xt k8s activation-smoke --context example-cluster --namespace tessera \
...
cargo xt k8s activation-smoke --context example-cluster --namespace tessera \
  --expected-image registry.example.com/example/tessera:v2026.05.2 \
  --require-target-worker \
  --local-orch-port 6602 \
  --local-orch-metrics-port 6603 \
  --local-gateway-port 4602 \
  --local-gateway-metrics-port 4603 \
  --out .dev/reports/guarded-kubernetes-expected-image-negative.json
cargo xt k8s activation-report-check \
  --report .dev/reports/guarded-kubernetes-expected-image-negative.json
cargo xt k8s activation-report-check \
  --report .dev/reports/guarded-kubernetes-readiness-negative.json \
...
- `.dev/reports/split-activation-plan-latest.json`: `status=ready`,
  ...
  and `remaining_uncovered=["guarded_kubernetes_activation_smoke"]`.
...
The guarded Kubernetes completion run wrote
`.dev/reports/guarded-kubernetes-activation-smoke-latest.json` with:
...
- all runtime deployments on
  `registry.example.com/example/tessera:v2026.05.2`
...
The private deployment smoke verified split publish, failure observation,
operator recovery, and post-smoke default-off cleanup. Environment identity,
registry coordinates, image digest, rollout revisions, topology inventory, and
controller status remain in the private deployment evidence store rather than
this public repository or its CI logs.

deployment validation used before applying the controlled smoke topology:

```sh
make validate-files FILES='<private-gitops-runtime-manifest>'
make validate
...
commit because its Helm template hook attempted to rewrite unrelated infrastructure
...
4. Persistent split state guarded Kubernetes restart evidence remains P6
