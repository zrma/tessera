...
- Orchestrator has an inactive split/merge planner skeleton, dry-run preview endpoint, and a default-off manual split activation RPC. The manual path now prepares target Workers, asks the source Worker to partition parent snapshot/buffered moves into child replay payloads, and publishes child assignments atomically after replay ack success. Automatic planner submission, merge activation, multi-depth split, and long-running post-publish convergence assertions remain deferred.
