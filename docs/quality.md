...
- Orchestrator has an inactive split/merge planner skeleton, dry-run preview endpoint, planner-to-operator `cargo xt split-activation-plan` helper, and a default-off manual split activation RPC. The planner-to-operator helper reads preview plus health/listing, writes mutation-free operator evidence, and is covered by `cargo xt dev activation-plan-smoke`. The manual path now prepares target Workers, asks the source Worker to partition parent snapshot/buffered moves into child replay payloads, publishes child assignments atomically after replay ack success, and has `cargo xt dev activation-smoke` coverage for local Gateway route convergence, source/target Worker owned-cell refresh, target relay replay, stable-session post-split Move, and remote AOI resync snapshot. `cargo xt dev activation-failure-smoke` covers local post-publish target outage detection, no automatic rollback observation, and target Worker restart recovery. `cargo xt dev activation-soak` covers local sustained child Ping/Move traffic, route convergence retention, remote AOI frame observation, Gateway latency histogram growth, and zero Gateway client close counters. `cargo xt k8s activation-smoke` provides a guarded internal MicroK8s operator helper for service port-forward, plan-only evidence, explicit publish, and optional target Worker scale-down/up recovery evidence; the 2026-05-02 `v2026.05.2` run verified internal cluster split publish, target-only failure detection, and target Worker recovery. The P5 split-activation rollback policy is `operator_recovery_no_automatic_merge_rollback_v1`: no automatic merge rollback, target Worker restoration as recovery, GitOps backout for controlled smoke rollback, and runtime merge activation deferred. Automatic planner submission, merge activation implementation, and multi-depth split remain deferred outside P5.
