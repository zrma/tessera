...
10. Guarded Kubernetes smoke covers the runtime-affecting cadence path after image
    publish and deployment rollout, then restores mutating flags to default-off and
    leaves deployment controller `Synced / Healthy`.
...
| Internal rollout | private deployment smoke window | image publish, rollout rev, cleanup rev, deployment controller `Synced / Healthy`, live smoke, and default-off cleanup are verified |
...
deployment rollout/default-off report and guarded Kubernetes controlled cadence
...
4. `v2026.05.7` runtime image publish, private deployment rollout, deployment controller
   `Synced / Healthy`, and deployment image match evidence.
5. Guarded Kubernetes controlled cadence smoke followed by default-off cleanup.
...
8. **Image and deployment rollout**: publish a new runtime image, promote through
   the private deployment source of truth, wait for deployment controller `Synced / Healthy`, and verify image
   match.
9. **Internal controlled cadence smoke**: open a deployment smoke window with
   ...
   `cargo xt k8s p8-cadence-smoke --context example-cluster --namespace tessera
   ...
   soak, and close observation. After the deployment cleanup revision removes the
   mutating env and preview fixture, run `cargo xt k8s
   p8-cadence-cleanup-check --context example-cluster --namespace tessera
   --expected-image <new-tag>` to finalize
   `.dev/reports/guarded-kubernetes-p8-cadence-smoke-latest.json`.
...
- Runtime-affecting slices must follow local smoke -> image publish -> deployment
  rollout -> deployment controller health -> internal smoke -> default-off cleanup.
