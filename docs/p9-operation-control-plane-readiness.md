...
must be followed by deployment cleanup that restores manual execution/activation and
...
6. A P9 runtime image is published and promoted through the private deployment source of truth with
   deployment controller `Synced / Healthy`, deployment image match, and default-off cleanup
   evidence.
7. Guarded Kubernetes recommend-mode soak runs against live Worker metrics and
...
| deployment rollout | private deployment image promotion | `.dev/reports/p9-gitops-rollout-latest.json` records image publish, rollout rev, cleanup rev, deployment controller health, and default-off cleanup |
| Internal recommend soak | `cargo xt k8s p9-recommend-soak` | `.dev/reports/guarded-kubernetes-p9-recommend-soak-latest.json` proves live recommend mode, durable history, replay, image match, and no mutation |
| Controlled spot-check | `cargo xt k8s p9-controlled-spot-check-report --source-report .dev/reports/guarded-kubernetes-operation-smoke-latest.json` | `.dev/reports/guarded-kubernetes-p9-controlled-spot-check-latest.json` proves approval, bounded execution, observation, replay after restart, and cleanup |
...
report, guarded Kubernetes recommend soak, controlled operation restart
...
5. **Image publish and deployment rollout**: publish a P9 runtime image, promote it
   through the private deployment source of truth, wait for deployment controller `Synced / Healthy`, and record a
   P9 rollout/default-off report.
6. **Internal recommend soak**: run `cargo xt k8s p9-recommend-soak` in
   guarded Kubernetes against live Worker metrics and durable storage, then
...
8. **Completion audit**: close with `cargo xt p9-completion-audit --json`,
   logical commits/pushes, CI verification, deployment controller health, and final default-off
   state.
...
- Controlled windows must be followed by deployment cleanup and audit evidence that
