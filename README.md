...
- GitHub Actions image publish workflow는 build/push only로 `v2026.05.4` Harbor image까지 발행했고, k8s GitOps repo에서 해당 tag로 promotion되어 ArgoCD `Synced / Healthy`, deployment image match, Gateway Ping smoke, P7 operation ledger persistence endpoint까지 확인했다. P6 controlled smoke coverage는 `v2026.05.3`에서 internal split/merge/multi-depth activation success/failure/restart/soak smoke까지 통과했다.
...
- 다음 범위는 P7 closed-loop operation loop다. Live Worker metrics와 assignment state에서 proposal을 지속적으로 만들되, durable operation record, explicit approval, policy/cooldown/budget gate를 통과한 작업만 executor가 mutation을 수행한다. 현재 local/dev evidence는 default-off split/merge/multi-depth operation loop와 approved same-Worker merge execution까지이며, internal `v2026.05.4` baseline은 operation ledger persistence와 default-off rollout까지만 확인했다. Observation/recovery/restart/soak/internal approved execution audit는 후속 gate다. 상세 목표와 prompt-to-artifact checklist는 `docs/p7-operation-loop.md`에 둔다.
- P0/P1/P2/P3/P4.1/P4.2 완료 기록은 `docs/completed-milestones.md`에 둔다.
- 현재 milestone decision gate와 rollout follow-up은 `docs/todo-next.md`, `docs/p7-operation-loop.md`, `docs/p6-completion-audit.md`, `docs/todo-p4-next-milestones.md`에 둔다.
...
- P7 operation loop: `TESSERA_ORCH_OPERATION_LEDGER_PATH`가 설정되면 Orchestrator metrics endpoint가 `GET /operations`, `POST /operations/proposals`, `POST /operations/approvals`, `POST /operations/executions`를 제공한다. Proposal/approval은 assignment mutation 없이 durable ledger에 기록되고, execution은 proposal hash, approval TTL, policy id, cooldown, budget, `TESSERA_ORCH_OPERATION_EXECUTION=manual`, 기존 `TESSERA_ORCH_SPLIT_MERGE_ACTIVATION=manual` gate를 모두 통과해야 runtime mutation을 시도한다. 현재 manual executor coverage는 approved same-Worker merge publish와 repeat idempotency까지이며, split/canonical multi-depth execution, observation, failure/recovery, restart, soak, internal MicroK8s rollout은 P7 후속 gate다.
...
- `cargo xt p6-completion-audit`는 `.dev/reports`의 internal report JSON을 모아 P6+ 완료 여부를 기계적으로 판정한다. 현재 P6+ 완료 상태에서는 `complete=true`가 정상이다. Internal planner mutation evidence는 `cargo xt k8s planner-activation-report`/`planner-activation-report-check` schema를 사용한다. P7 closed-loop operation evidence와 audit 기준은 `docs/p7-operation-loop.md`에서 시작한다.
