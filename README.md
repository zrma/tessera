...
- 리밸런싱 자동화와 자동 split/merge 런타임은 아직 미구현이다. Orchestrator에는 비활성 split/merge planner skeleton, fixture-backed dry-run preview smoke, 그리고 P4.3 slice로 default-off/manual `SubmitSplitActivation` replay/publish surface가 있다. 이 surface는 split-only target map/depth/registration validation, Worker replay prepare/request/ack, atomic child assignment publication, pre-publication rollback을 제공한다. Merge activation, automatic planner submission, multi-depth split, post-publish automatic merge rollback은 아직 미구현이다.
