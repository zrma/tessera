...
- 리밸런싱 자동화와 자동 split/merge 런타임은 아직 미구현이다. Orchestrator에는 비활성 split/merge planner skeleton, fixture-backed dry-run preview smoke, planner-to-operator plan helper, 그리고 P4.3 slice로 default-off/manual `SubmitSplitActivation` replay/publish surface가 있다. 이 surface는 split-only target map/depth/registration validation, Worker replay prepare/request/ack, atomic child assignment publication, pre-publication rollback, local two-Worker route convergence/remote AOI resync smoke, post-publish target outage detection/restart recovery smoke, local load/soak observation smoke, internal MicroK8s port-forward smoke helper를 제공한다. P5 split activation은 automatic merge rollback 없이 target Worker restoration + GitOps backout을 운영 정책으로 삼는다. Merge activation, automatic planner submission, multi-depth split, 실제 internal cluster activation evidence는 아직 미구현/미검증이다.
