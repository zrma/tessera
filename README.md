...
- Internal MicroK8s activation smoke:
  ...
  - Internal multi-depth readiness helper: `cargo xt k8s multi-depth-activation-smoke --context microk8s-ts --namespace tessera --expected-image <new-tag>` (canonical parent `world=0,cx=-2,cy=3,depth=2,sub=0`와 explicit child target map을 live Orchestrator listing에 대해 검증하고 `.dev/reports/internal-microk8s-multi-depth-activation-smoke-latest.json`에 read-only plan/block evidence를 남긴다)
  - Internal multi-depth readiness verifier: `cargo xt k8s multi-depth-activation-report-check --require-ready-plan --expected-image <new-tag>` (read-only internal canonical multi-depth report가 mutation 없이 ready parent/child target map과 `--target-cell` submission evidence를 포함하는지 검증한다. 승인된 publish report가 생기면 `--require-published --require-failure --require-restart --require-soak`로 canonical multi-depth publish/failure/restart/load-soak evidence까지 검증한다)
  ...
  - P6+ completion audit: `cargo xt p6-completion-audit --json` (P5 split baseline은 통과시키고, P6 internal restart/live-metrics/GitOps rollout/merge/multi-depth/planner mutation evidence가 모두 갖춰지기 전에는 nonzero로 남은 gate를 출력한다. GitOps rollout과 planner mutation gate는 각각 전용 report schema까지 검증한다)
...
- 리밸런싱 자동화와 자동 split/merge 런타임은 아직 미구현이다. Orchestrator에는 비활성 split/merge planner skeleton, fixture-backed dry-run preview smoke, planner-to-operator split/merge plan helper, live Worker metrics 기반 split planner-to-operator helper, policy-gated `planner-activation` helper, P4.3/P5 default-off/manual `SubmitSplitActivation` replay/publish surface, P6 manual `SubmitMergeActivation` same-Worker coalescing + cross-Worker child replay surface, 그리고 P6 첫 slice인 opt-in durable assignment state가 있다. 이 surface는 split-only target map/depth/registration validation, Worker replay prepare/request/ack, atomic child assignment publication, legacy/canonical merge sibling ownership/registration validation, Worker child-to-parent coalescing, remote child-to-parent replay, pre-publication rollback, local two-Worker route convergence/remote AOI resync smoke, live metrics plan 기반 manual submission smoke, policy-approved merge planner mutation smoke, live metrics split planner mutation smoke, merge activation/cross-worker/failure/restart/soak smoke와 report checker, canonical explicit-child local split success/failure/restart/soak smoke와 report checker, canonical merge local success/failure/restart/soak smoke와 report checker, post-publish target outage detection/restart recovery smoke, Orchestrator restart recovery smoke, local load/soak observation smoke, internal MicroK8s port-forward smoke helper와 verified P5 internal cluster success/failure/recovery evidence를 제공한다. P6 persistent restart path에는 internal helper/verifier와 PVC-backed GitOps draft가 준비됐고, internal live metrics plan path에는 `--use-live-worker-metrics` helper와 `--require-live-metrics-plan` verifier가 준비됐다. Internal merge path에는 read-only `cargo xt k8s merge-activation-smoke`와 `merge-activation-report-check --require-ready-plan` verifier가 준비됐지만, 실제 publish/failure/restart/soak cluster evidence는 아직 닫히지 않았다. Internal multi-depth path에는 read-only `cargo xt k8s multi-depth-activation-smoke`와 `multi-depth-activation-report-check --require-ready-plan` verifier가 준비됐지만, 실제 canonical multi-depth publish/failure/restart/soak cluster evidence는 아직 닫히지 않았다. 실제 internal P6 evidence는 새 image publish와 승인된 GitOps rollout 뒤에 닫아야 한다. P5 split activation은 automatic merge rollback 없이 target Worker restoration + GitOps backout을 운영 정책으로 삼는다. Planner mutation internal evidence, merge internal evidence, canonical merge internal evidence, multi-depth internal evidence는 후속 범위다.
...
- P5 completion audit와 prompt-to-artifact checklist는 `docs/p5-completion-audit.md`에 둔다. 현재 P6+ completion audit와 missing-gate checklist는 `docs/p6-completion-audit.md`에 둔다.
...
- `cargo xt p6-completion-audit`는 `.dev/reports`의 internal report JSON을 모아 P6+ 완료 여부를 기계적으로 판정한다. 현재는 남은 internal P6+ gate 때문에 실패하는 것이 정상이다. Internal planner mutation evidence는 `cargo xt k8s planner-activation-report`/`planner-activation-report-check` schema를 사용한다.
