...
- GitHub Actions image publish workflow는 build/push only로 추가됐지만, Harbor push는 repository secret 또는 Harbor 계정 권한 정정 후 재실행해야 한다. Runtime GitOps tag promotion은 k8s GitOps repo에서 별도 커밋으로 진행한다.
