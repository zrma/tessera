...
- Manual split activation helper:
  ...
  - `cargo xt split-activation --operation-id split-001 --target 0=worker-a --target 1=worker-b --target 2=worker-a --target 3=worker-b` (`--orch-addr`, `--world`, `--cx`, `--cy`로 대상 조정; Orchestrator는 `TESSERA_ORCH_SPLIT_MERGE_ACTIVATION=manual`이어야 publish 가능)
...
- 환경변수(옵션):
  ...
  - `TESSERA_ORCH_SPLIT_MERGE_ACTIVATION` 기본 unset/disabled (`manual`로 설정한 경우에만 `SubmitSplitActivation` gRPC가 split-only replay/publish activation을 수행)
  - `RUST_LOG` 기본 `info`
...
- Orchestrator/Gateway: Orchestrator는 `RegisterWorker`/`GetAssignments`/`ListAssignments`/`WatchAssignments`와 `GetHealth`/`GetMetrics`/`SubmitHandoverCommand`/`SubmitSplitActivation` gRPC 엔드포인트를 제공한다. `SubmitSplitActivation`은 default-off manual flag 뒤에서 split-only target map/depth/registration validation, target Worker split replay prepare, source Worker parent snapshot/buffered move partition replay, 모든 child replay 성공 후 parent removal + child assignment atomic publish를 수행하며, replay/prepare 실패 시 parent assignment를 유지하고 staged target payload를 abort한다. `cargo xt split-activation-plan`은 `GET /split-merge/preview` dry-run 결과와 Orchestrator health/listing을 읽어 activation mutation 없이 operator review용 target map, precondition, submit command를 기록한다. `cargo xt dev activation-smoke`는 이 경로에서 Gateway route convergence, source/target Worker child ownership refresh, stable Gateway session의 post-split Move, remote child AOI resync snapshot을 로컬 두-Worker dev stack에서 검증한다. `cargo xt dev activation-failure-smoke`는 post-publish target Worker outage를 감지하고, 자동 rollback 없이 operator recovery가 필요함을 evidence로 남긴 뒤 target Worker 재기동 복구를 검증한다. `cargo xt dev activation-soak`은 split publish 후 반복 child Ping/Move traffic, route convergence 유지, remote AOI frame 관측, Gateway latency histogram/close counter를 로컬 evidence로 기록한다. `cargo xt k8s activation-smoke`는 승인된 internal two-Worker topology에서 같은 operator flow를 port-forward로 실행하고, 명시 flag 없이는 mutation 전 plan-only로 중단한다. P5 split activation의 rollback policy는 `operator_recovery_no_automatic_merge_rollback_v1`로 고정되어 automatic merge rollback은 하지 않고 target Worker 복구와 GitOps smoke slice backout을 운영자 절차로 검증한다. Orchestrator/Gateway 모두 opt-in Prometheus `/metrics` exporter를 제공하며 Orchestrator는 assignment 변경 없는 `GET /split-merge/preview` dry-run endpoint를 제공한다. Gateway는 route availability 기반 `/ready`, reconnect/close-reason counters, Ping/Pong round-trip latency histogram, request-id로 매칭된 Join/Move round-trip latency histogram을 노출
...
- GitHub Actions image publish workflow는 build/push only로 `v2026.05.2` Harbor image까지 발행했고, k8s GitOps repo에서 해당 tag로 controlled smoke promotion되어 ArgoCD sync와 internal split activation success/failure/recovery smoke까지 통과했다.
- 리밸런싱 자동화와 자동 split/merge 런타임은 아직 미구현이다. Orchestrator에는 비활성 split/merge planner skeleton, fixture-backed dry-run preview smoke, planner-to-operator plan helper, 그리고 P4.3/P5 slice로 default-off/manual `SubmitSplitActivation` replay/publish surface가 있다. 이 surface는 split-only target map/depth/registration validation, Worker replay prepare/request/ack, atomic child assignment publication, pre-publication rollback, local two-Worker route convergence/remote AOI resync smoke, post-publish target outage detection/restart recovery smoke, local load/soak observation smoke, internal MicroK8s port-forward smoke helper와 verified internal cluster success/failure/recovery evidence를 제공한다. P5 split activation은 automatic merge rollback 없이 target Worker restoration + GitOps backout을 운영 정책으로 삼는다. Merge activation, automatic planner submission, multi-depth split은 후속 범위다.
...
- P5 completion audit와 prompt-to-artifact checklist는 `docs/p5-completion-audit.md`에 둔다.
...
- Split activation control-plane: `SubmitSplitActivation`은 `TESSERA_ORCH_SPLIT_MERGE_ACTIVATION=manual`에서만 split-only 명령을 실행한다. 요청은 `depth=0/sub=0` parent와 `sub=0..3` 전체 target map을 요구하고, source ownership/registration, configured/registered target Worker, no-op source-only target map, active handover, 이미 published 된 child overlap, 이미 staged 된 split family를 검증한다. 성공 경로는 target Worker에 child replay를 pre-stage하고 source Worker에 parent snapshot/buffered move partition replay를 요청한 뒤, 모든 child replay ack가 성공할 때만 parent assignment 제거와 child assignment publish를 한 번에 수행한다. 실패 경로는 `assignments_changed=false`로 parent assignment를 유지하고 prepared child payload를 abort한다.
...
- `cargo xt split-activation-plan`은 preview dry-run output과 Orchestrator health/listing을 읽어 operator review용 `.dev/reports/split-activation-plan-latest.json`을 만든다. 이 report는 `activation_mutated=false`, 선택된 split candidate, registered worker 상태, 권장 `sub=worker-id` target map, precondition, `cargo xt split-activation ...` command template을 포함한다. `cargo xt split-activation`은 operator가 explicit `sub=worker-id` target map으로 `SubmitSplitActivation`을 호출하는 수동 실행 helper다. `cargo xt dev activation-plan-smoke`는 preview candidate가 mutation 없이 ready operator plan으로 변환되는지 확인한다. `cargo xt dev activation-smoke`는 별도 포트의 two-Worker dev stack에서 `TESSERA_ORCH_SPLIT_MERGE_ACTIVATION=manual`을 켜고 같은 RPC를 호출한 뒤, Orchestrator listing의 4-child publish, Gateway `/ready` route count 4, 각 child Ping, stable Gateway session route switch, target Worker relay replay metric, remote child AOI resync snapshot을 확인하고 `.dev/reports/activation-smoke-latest.json`에 evidence summary를 쓴다. `cargo xt dev activation-failure-smoke`는 post-publish target Worker outage, failed child convergence, no automatic rollback, target Worker restart recovery를 `.dev/reports/activation-failure-smoke-latest.json`에 기록한다. `cargo xt dev activation-soak`은 child별 반복 Ping/Move traffic, remote AOI frame 관측, Gateway route/latency/close counter를 `.dev/reports/activation-soak-latest.json`에 기록한다. `cargo xt k8s activation-smoke`는 internal MicroK8s service port-forward, plan-only guard, optional split publish, optional target Worker scale-down/up recovery check를 `.dev/reports/internal-microk8s-activation-smoke-latest.json`에 기록한다.
