...
- 다음 범위는 P7 closed-loop operation loop다. Live Worker metrics와 assignment state에서 proposal을 지속적으로 만들되, durable operation record, explicit approval, policy/cooldown/budget gate를 통과한 작업만 executor가 mutation을 수행한다. 상세 목표와 prompt-to-artifact checklist는 `docs/p7-operation-loop.md`에 둔다.
...
- `cargo xt p7-operation-ledger-check --ledger .dev/operation-ledger.json --require-approval --require-blocked-execution`은 P7 ledger가 proposal, durable approval, default-off blocked execution evidence를 포함하는지 검증한다. 이 검증은 runtime execution/observation/recovery/internal rollout evidence를 대체하지 않는다.
