...
enum K8sSub {
    ...
    OperationSmoke {
        ...
        #[arg(long, default_value_t = 10)]
        soak_sleep_ms: u64,
        /// Output JSON path. Defaults to .dev/reports/internal-microk8s-p7-operation-smoke-latest.json
        #[arg(long)]
        out: Option<PathBuf>,
    },
    /// Validate an internal P7 operation smoke report JSON
    OperationReportCheck {
        /// Report JSON path. Defaults to .dev/reports/internal-microk8s-p7-operation-smoke-latest.json
        #[arg(long)]
        report: Option<PathBuf>,
...
fn dev_p7_operation_loop_smoke() -> Result<()> {
    ...
    let report = serde_json::json!({
        ...
        "remaining_uncovered": [
            ...
            "restart_recovery",
            "soak",
            "internal_microk8s_operation_loop_smoke",
            "p7_completion_audit"
        ]
    });
...
fn dev_p7_operation_execution_smoke() -> Result<()> {
    ...
    let report = serde_json::json!({
        ...
        "remaining_uncovered": [
            ...
            "restart_recovery",
            "soak",
            "internal_microk8s_operation_execution_smoke",
            "p7_completion_audit"
        ]
    });
...
fn dev_p7_operation_observation_smoke() -> Result<()> {
    ...
    let report = serde_json::json!({
        ...
        "remaining_uncovered": [
            ...
            "restart_recovery",
            "soak",
            "internal_microk8s_operation_observation_smoke",
            "p7_completion_audit"
        ]
    });
...
fn dev_p7_operation_recovery_smoke() -> Result<()> {
    ...
    let report = serde_json::json!({
        ...
        "remaining_uncovered": [
            ...
            "restart_recovery",
            "soak",
            "internal_microk8s_operation_recovery_smoke",
            "p7_completion_audit"
        ]
    });
...
fn dev_p7_operation_soak_smoke(iterations: u32, sleep_ms: u64) -> Result<()> {
    ...
    let report = serde_json::json!({
        ...
        "remaining_uncovered": [
            "split_runtime_execution",
            "multi_depth_runtime_execution",
            "internal_microk8s_operation_soak_smoke",
            "p7_completion_audit"
        ]
    });
...
fn write_internal_k8s_operation_report(
    ...
    if !completion.operation_recorded {
        remaining_uncovered.push("internal_microk8s_operation_proposal_smoke");
    }
    if !completion.execution_published {
        remaining_uncovered.push("internal_microk8s_operation_execution_smoke");
    }
    if !completion.observation_completed {
        remaining_uncovered.push("internal_microk8s_operation_observation_smoke");
    }
    if !completion.load_soak_ran {
        remaining_uncovered.push("internal_microk8s_operation_soak_smoke");
    }
    if !completion.observation_recovery_required {
        remaining_uncovered.push("internal_microk8s_operation_failure_recovery_smoke");
    }
    if !completion.orchestrator_restart_smoke_ran {
        remaining_uncovered.push("internal_microk8s_operation_restart_smoke");
    }
    remaining_uncovered.push("p7_completion_audit");

    let report = serde_json::json!({
        "schema": "tessera.internal_microk8s_p7_operation_smoke.v1",
        "unix_ts": unix_timestamp_secs(),
        "stage": input.stage,
...
fn validate_p7_operation_loop_smoke_report(report: &serde_json::Value) -> Result<()> {
    ...
    assert_remaining_uncovered_contains(report, "internal_microk8s_operation_loop_smoke")?;
    Ok(())
}
...
fn validate_p7_operation_observation_smoke_report(report: &serde_json::Value) -> Result<()> {
    ...
    assert_remaining_uncovered_contains(report, "internal_microk8s_operation_observation_smoke")?;
    Ok(())
}
...
fn validate_p7_operation_soak_smoke_report(
    ...
    assert_remaining_uncovered_contains(report, "internal_microk8s_operation_soak_smoke")?;
    Ok(())
}
...
fn p6_restart_preflight_evidence(report_dir: &Path) -> Option<String> {
    ...
    for report_name in [
        "internal-microk8s-restart-readiness-preflight.json",
        "internal-microk8s-restart-readiness-negative.json",
    ] {
        let path = report_dir.join(report_name);
...
fn validate_internal_k8s_operation_report(
    ...
    assert_json_str_eq(
        report,
        &["schema"],
        "tessera.internal_microk8s_p7_operation_smoke.v1",
    )?;
    let stage = json_str(report, &["stage"])?;
    match stage {
    ...
    if stage == "planned_without_execution" || stage == "completed" || stage == "recovery_required"
    {
        ...
        assert_remaining_uncovered_absent(report, "internal_microk8s_operation_proposal_smoke")?;
    }
        ...
        || requirements.require_restart
    {
        ...
        assert_remaining_uncovered_absent(report, "internal_microk8s_operation_execution_smoke")?;
    }
    ...
    if requirements.require_soak {
        ...
        assert_remaining_uncovered_absent(report, "internal_microk8s_operation_soak_smoke")?;
    }
    if requirements.require_restart {
        ...
        let ledger = json_field(report, &["ledger", "snapshot"])?;
        validate_p7_operation_ledger(ledger, true, false, true, true, false)?;
        assert_remaining_uncovered_absent(report, "internal_microk8s_operation_restart_smoke")?;
    }
    if requirements.require_recovery_required {
        ...
        assert_remaining_uncovered_absent(
            report,
            "internal_microk8s_operation_failure_recovery_smoke",
        )?;
    }
    Ok(())
}
...
"#;
    ...
    fn p6_completion_audit_reports_missing_internal_gates() {
        ...
        fs::write(
            dir.join("internal-microk8s-restart-readiness-preflight.json"),
        ...
        assert!(findings.iter().any(|finding| {
            finding.gate == "p6_internal_restart_recovery"
                && finding
                    .evidence
                    .contains("internal-microk8s-restart-readiness-preflight.json")
                && finding
                    .evidence
